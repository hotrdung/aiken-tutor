...
// cannot use local BF (ryo) to submit tx
...
export const wallet = new MeshWallet({
  ...
  // -- preview
  // submitter: bfProvider,

  // ==================================
  // key: {
  //   type: "root",
  //   bech32: fs.readFileSync("me-mesh.sk").toString(),
  // },
  key: {
    type: "cli",
    // 'payment': fs.readFileSync("me.sk").toString(),
    // (!!) me.sk is CLI sk in JSON format
    // -> use field `cborHex`, without prefix "5820" -> 32-byte length
    "payment": "a14d0381b7c9e56f910475ba74f6eff647a2351e2f12e0171a8bb12ecf8a4c8f",
  },
});
...
export async function getPParams() {
  ...
  // (!!) with the current MeshJS lib, `pp` does not include `costModels` yet
  // --------- the following line has not work yet! (need more modifications to the lib) ----------
  // pp.costModels = {
  //   PlutusV1: [],
  //   PlutusV2: [],
  //   PlutusV3: [
